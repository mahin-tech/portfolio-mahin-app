...
const Portfolios = ({ portfolios: initialPortfolios }) => {
    ...
    const _deletePortfolio = async (e, portfolioId) => {
        ...
        if (isConfirm) {
            ...
            setPortfolios(portfolios.portfolio.filter(p => p._id !== portfolioId));
        }
    }

    return (
        <BaseLayout user={dataU} loading={loadingU}>
            <BasePage header="Portfolios" className="portfolio-page" title="Newest Portfolios - Mahin Prajapati">
                <Row>
                    {portfolios.portfolio.map(portfolio =>
                        <Col
                            key={portfolio._id}
                            onClick={() => {
